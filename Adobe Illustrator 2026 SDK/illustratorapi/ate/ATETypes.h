...
enum StreamVersion
{
    // see more comments in TextStreamVersion
    ...
    kStreamVersion9,                                                        // CC (ATE 7.x). Ai new features: new font size UX, frame vertical alignment, soft bottom frames, Bullets & Numbers. Hz new feature: Magic Text
    kStreamVersion10,                                                       // ATE 7.3.0, composition protector
    kStreamVersion11,                                                       // ATE 7.3.1
    kStreamVersion12,                                                       // ATE 7.9.0, List Label Styles
    kStreamVersion13,                                                       // ATE 7.10.0, update magic word gap
...
/** Adobe Text Engine (ATE) text vertical alignment type constants */
enum FrameAlignment
{
    kTopAlignment = 0,
    kCenterAlignment,
...
#ifndef __cplusplus
typedef enum FrameAlignment FrameAlignment;
#endif

// ----------------------------------------------------------------------------------

/** Adobe Text Engine (ATE) text Frame Fitting Options */

enum FrameFitPolicy
{
    kDontAutoFit = 0,

	/*Changing the height of path for fitting*/
	kHeightExtensionWithCursorHeight = 1,
    kHeightExtensionWithPreciseBounds,
    kHeightExtensionWithBaseline,

    /*
     *   This flag does not require multiple compositions and no client call back 
     *   to determine the final height of a frame.
     *   It is the most efficient one from this set.
     */
    kSoftBottomWithCursorHeight,

/*Below option can be added as well, but for the time being
are not supported as now*/
#if 0
    /*Changing the height of path for fitting*/
	kWidthExtension,
    /*Changing the height and width of path uniformly*/
    kUniformExtension,
	/*Change the Font Size of text element for Fitting rather than changing the path*/
	kSizeExtension
    /*we can also have a custom fitting policy which is controlled completely
    by client of ATE*/
#endif
};

inline constexpr bool IsSoftBottomFitPolicy(const FrameFitPolicy policy)
{
    return policy == kSoftBottomWithCursorHeight;
}

#ifndef __cplusplus
typedef enum FrameFitPolicy FrameFitPolicy;
#endif
...
/** Adobe Text Engine (ATE) line height type constants */
enum LineHeightType
{
    kEMBoxLineHeightType = 0,           // default: EM box height determines line height
    kPointSizeLineHeightType,           // point size determines line height
};
#ifndef __cplusplus
typedef enum LineHeightType LineHeightType;
#endif
...
/** Adobe Text Engine (ATE) Font Height Option Reference .
This is also being referred as setting the reference type which will
also guide how to evaluate reference font size value.*/
enum FontSizeRefType
{
    kFontSizeRefDefault, //Embox
    kFontSizeRef_CapHeight,
    kFontSizeRef_XHeight,
    kFontSizeRef_ICFBox
};

#ifndef __cplusplus
typedef enum FontSizeRefType FontSizeRefType;
#endif
...
enum Language
{
    kEnglishLanguage,                   // US English
    kFinnishLanguage,
    kStandardFrenchLanguage,            // European French / France French
    ...
    kGerman1901ReformLanguage,          // As we moved from Proximity Hunspell the Old German language mapping changed to 1901 language.
    kSwissGerman1996Language,           // Pre 2006 Reform.  Previously kSwissGermanLanguage.  See below for latest Swiss German. Note: Standard German as spoken in Switzerland, *not* Schweizerdeutsch
    kItalianLanguage,
    kBokmalNorwegianLanguage,
    kNynorskNorwegianLanguage,
    kStandardPortugueseLanguage,        // European Portuguese
    kBrazilianPortugueseLanguage,
    kSpanishLanguage,
    kSwedishLanguage,
    kUKEnglishLanguage,
    kDutch1995Language,                  // Pre 2006 Reform. Previously kDutchLanguage. See below for latest Dutch.
    kDanish,
    kCatalan,
    kRussian,
    kUkrainian,
    ...
    kSwissGerman2006ReformLanguage,     // Latest Swiss German.   Should be the default as of 2006. Note: Standard German as spoken in Switzerland, *not* Schweizerdeutsch
    kCanadianEnglish,
    kThai,
    kVietnamese,
    kLao,
    kHindi,
    kMarathi,
    kBengaliIndia,                      // a.k.a. Bangla (India)
    kPunjabi,
    kGujarati,
    kOriya,                             // a.k.a. Odia
    kTamil,
    ...
    kKannada,
    kMalayalam,
    kBurmese,                           // a.k.a. Myanmar
    kSinhalese,                         // a.k.a. Sinhala
    kKhmer,
    kIndonesian,
    kKoreanLanguage,
    kChineseSimplified,
    kChineseHongKong,
    kNepali,                          // a.k.a. Nepalese
    kSanskrit,
    // [ATE-4106] These languages should be added in order to have 1:1 mapping with LILO
    // when a client asks for all available dictionaries form a pfovider (e.g. Hunspell)
    kGermanLanguage,
...
// old enum names, to keep backward compatibility
constexpr Language kBrazillianPortugueseLanguage = kBrazilianPortugueseLanguage;
constexpr Language kUkranian = kUkrainian;
constexpr Language kRumanian = kRomanian;
constexpr Language kChineseLanguage = kChineseTraditional;
constexpr Language kBelarussian = kBelarusian;
constexpr Language kOldGermanLanguage = kGerman1901ReformLanguage;
constexpr Language kOldSwissGermanLanguage = kSwissGerman1996Language;
constexpr Language kOldDutchLanguage = kDutch1995Language;
...
/**
 * [gfrancis]: ProjectX introduced the Unified Text Object, which requires different
 * alignment behavior for ATE's PointText frame type.
 *
 * The enum should only be used for Point Text.
 */
enum PointTextAlignmentMode
{
    kPointTextAlignOnOrigin     = 0,
    kPointTextAlignOnFrameWidth = 1
};

#ifndef __cplusplus
typedef enum PointTextMode PointTextMode;
#endif
...
enum MojikumiTablePredefinedTag
{
    ...
    kPredefinedGyomatsuYakumonoZenkakuOrHankakuMojikumiTableTag = 5,
    kPredefinedGyomatsuYakumonoHankakuV2MojikumiTableTag = kPredefinedGyomatsuYakumonoHankakuMojikumiTableTag, // backward compatibility
    kPredefinedTsumeGumiMojikumiTableTag = 6,
    kPredefinedBetaGumiMojikumiTableTag = 7,
};
#ifndef __cplusplus
...
enum ListStylePredefinedTag
{
    ...
    kPredefinedSingleZeroPrefixNumericListStyleTag = 7,
    kPredefinedDoubleZeroPrefixNumericListStyleTag = 8
};

// ----------------------------------------------------------------------------------

/** Adobe Text Engine (ATE) text list label alignment type constants */
// ----------------------------------------------------------------------------------
// [aburago 10-22-08] We use "start" and "end" here to emphasize logical alignment
//                      w.r.t. RTL placement
enum BNListAlignment
{
    kBN_AlignStart = 0,
    kBN_AlignCenter,
    kBN_AlignEnd
...
#ifndef __cplusplus
typedef enum BNListAlignment BNListAlignment;
#endif

// ----------------------------------------------------------------------------------

/** Adobe Text Engine (ATE) text list indent units  */
enum IndentUnits
{
    kEms = 0,
    kPoints,
...
#ifndef __cplusplus
typedef enum IndentUnits IndentUnits;
#endif


// ----------------------------------------------------------------------------------

/** Adobe Text Engine (ATE) text editing policies
 ...
 Example:
    ...
    kLoseListLabelStyleDefaultInheritance: When this option is enabled, the list label style will not inherit the style of the
    first character of the paragraph. Instead, the normal style will be used.
*/

enum ListStyleSettings
{
    kDefaultList                                        = 0,
    kDontBreakNumberingForSameSeqSameLevel              = 1 << 0,
    kUseOriginalFontTag                                 = 1 << 1,
    kHandleMultipleCROnAllLevelsNParas                  = 1 << 2,
    kDontAccumulateTextIndentFromPreviousLevels         = 1 << 3,
    kEnableListTierChange                               = 1 << 4,
    kEnableListStyleDetection                           = 1 << 5,
    kDeindentOnBackspaceKey                             = 1 << 6,
    kEnableExistingListStyleItemDetection               = 1 << 7,
    kKeepListLabelsWithinFrameBounds                    = 1 << 8,
    kDisableListStyleUniformizationForCopyPaste         = 1 << 9,
    kLoseListLabelStyleDefaultInheritance               = 1 << 10,
};
...
/** ATE Document policies regarding streaming data and predefined resources during Read/Write operation.
    ...
    kOmitMostDocumentSettings: do not stream out document settings like smart quote settings for every language.
    ...
    kRefResourcesByUUID: By default, streamed objects and resources refer the resource by its index in resource set.
        But by this policy, resources will be referred by their UUIDs instead of indexes. YOU MUST HAVE AN UUID GENERATOR IN PLACE
        This only matters for stream-out operation, stream-in always prefers UUIDs if they are available
    ...
    This only works for the stream out operation.
...
enum StreamingPolicy
{
    ...
    kOmitTextFrameAsResource    = 1 << 4,
    kIncludeOnlyGivenStream     = 1 << 5,
    kIncludeOnlyResources       = 1 << 6,
    kRefResourcesByUUID         = 1 << 7
...
/* To retrieve the text outlines for given range selectively determined by these policies  */
enum OutlineExtractionPolicy //a little strange
{
    /* All the text outlines are retrieved - This is the default case */
    kExtractAll                 = 0,

    /* This policy will retrieve all the text outlines omitting the Glyph Outlines */
    kOmitGlyphs                 = 1 << 0,

    /* This policy will retrieve all the text outlines omitting the Underline Annotation Outlines */
    kOmitUnderlines             = 1 << 1,

    /* This policy will retrieve all the text outlines omitting the StrikeThrough Annotation Outlines */
    kOmitStrikeThrough          = 1 << 2,

    /* This policy will forcefully extract the Underline Annotation Outlines of the given range as if the
     entire text has been underlined */
    kForceIncludeUnderlines     = 1 << 3,

    /* This policy will retrieve the text annotation (underlines/StrikeThrough) in the form of Path Only.
     (without stroke) It will not convert the annnotations paths to full polygons.*/
    kAnnotationNoStrokePath     = 1 << 4
};
...
/*
 SLOLilo enums
 To retrieve or check word based on equivalent forms or Not */
enum AddEquivalentFormsSetting { kDoNotAddEquivalentForms, kAddEquivalentForms };
enum CheckEquivalentFormsSetting { kDoNotCheckEquivalentForms, kCheckEquivalentForms };
enum AddOriginalWordSetting { kDoNotAddOriginalWord, kAddOriginalWord };

// ----------------------------------------------------------------------------------

#ifdef __cplusplus
ATE_ENUM_BIT_OPERATORS(StreamingPolicy)
ATE_ENUM_BIT_OPERATORS(OutlineExtractionPolicy)
ATE_ENUM_BIT_OPERATORS(ListStyleSettings)
#else
typedef enum StreamingPolicy StreamingPolicy;
typedef enum OutlineExtractionPolicy OutlineExtractionPolicy;
typedef enum ListStyleSettings ListStyleSettings;
#endif
