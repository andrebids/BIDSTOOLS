	...
	*/
	...
	public:
        ...
        */
        ...
        /** Sets the absolute ruler origin of the artboard.
        @param rulerOrigin The new ruler origin.
        */
        AIErr SetRulerOriginAbsolute(const AIRealPoint& rulerOrigin);
        
        /** Retrieves the absolute ruler origin of the artboard
        @param rulerOrigin [out] A buffer in which to return the ruler origin.
        */
        AIErr GetRulerOriginAbsolute(AIRealPoint& rulerOrigin) const;
#endif

		AIErr GetUUIDAsString(ai::UnicodeString& uuid) const;
		
		/** Set the color the artboard
		@param AIColor [out] A AIColor object to set color of artboard.
		*/
		AIErr SetColor(const AIColor& color);

		/** Retrieves the color the artboard
		@param AIColor [out] A AIColor object to get color of artboard.
		*/
		AIErr GetColor(AIColor& color) const;

		AIErr GetLocked(AIBoolean& isLocked) const;
		AIErr SetLocked(const AIBoolean& isLocked);

		AIErr GetHide(AIBoolean & isHidden) const;
		AIErr SetHide(const AIBoolean& isHidden);
	...
	*/
	namespace ArtboardUtils
	{
		...
		*/
		...
		/** Deselects all artboards.
		*/
		inline AIErr DeselectAllArtboards()
		{
			ArtboardList list;
		...
		*/
		...
		/** Retrieves the number of selected artboards in the current document
			@param count [out] A buffer in which to return the number of selected artboards.
		*/
		AIErr GetNumSelectedArtboards(ai::ArtboardID& count);
	}
}
